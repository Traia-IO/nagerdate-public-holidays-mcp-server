...
# Script to build and run the Nager.Date Public Holidays MCP Server locally in Docker
...
IMAGE_NAME="nagerdate-public-holidays-mcp-server"
CONTAINER_NAME="nagerdate-public-holidays-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Nager.Date Public Holidays MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Nager.Date Public Holidays MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ Nager.Date Public Holidays MCP Server is running!${NC}"
echo
