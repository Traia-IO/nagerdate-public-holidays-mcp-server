#!/usr/bin/env python3
"""
Nager.Date Public Holidays MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('nagerdate-public-holidays_mcp')
...
logger.info("="*80)
logger.info(f"Nager.Date Public Holidays MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://date.nager.at")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("Nager.Date Public Holidays MCP Server", host="0.0.0.0")
...
# - NAGERDATE_PUBLIC_HOLIDAYS_API_KEY: Server's internal API key for payment mode
#
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="nagerdate-public-holidays-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "nagerdate-public-holidays-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Nager.Date Public Holidays MCP Server")
    logger.info("="*80)
